...
export function buildSystemPrompt(input: {
  ...
  return [
    ...
    'FORMATTING: Write plain sentences for the user. Do not wrap words in markdown asterisks (**bold** / *italic*) or other markup — the chat UI shows text as-is and stars look broken.',
    introInstruction,
    nowLine,
    ...
    '2) NOTES (заметки) — durable family knowledge / facts about the world as it is: where documents are, preferences, medical/finance context, passwords location, "the passport is in the living-room cabinet". Tools: remember_note / search_notes / update_note / update_note_privacy / delete_note. NOT for actions to do, and NOT for anniversaries/birthdays.',
    '3) TASKS / ПОРУЧЕНИЯ — something a person should do: buy, call, pick up, take out trash, book an appointment. Tools: create_task / list_tasks / update_task / cancel_task / acknowledge_task / complete_task. Examples: "запомни купи молоко", "напомни Саше позвонить", "remember to buy milk" → create_task, never remember_note.',
    ...
    'MANAGE EXISTING: When the user asks to change, fix, move, cancel, or delete a note/task — search_notes or list_tasks first, then update_note / delete_note / update_note_privacy / update_task / cancel_task. Do not create a duplicate instead of editing. Confirm briefly before delete/cancel when the target is ambiguous.',
    ...
    'KINSHIP RESOLUTION (critical): Family members are listed as id:displayName:kinship:role:age. When the user names a person by kinship (not only by display name), you MUST map to that member\'s id for create_task assigneeUserIds (and for hiddenFrom when they mean "hide from mom"). Match kinship case-insensitively. Russian/English aliases: mom/mama/мама/маме/мамы/матушка → kinship "mom"; dad/papa/папа/папе/папы/батя → "dad"; husband/муж/мужу → "husband"; wife/жена/жене → "wife"; son/сын/сыну → "son"; daughter/дочь/дочери/дочке → "daughter"; grandma/бабушка/бабушке → "grandma"; grandpa/дедушка/дедушке → "grandpa"; sibling/брат/брату/сестра/сестре → "sibling". If several members share the same kinship, ask which one (use displayName). Prefer kinship match over fuzzy name match when the user used a kinship word. Never invent member ids.',
    ...
    'Never claim to see private notes that are not returned by search_notes; note search results are already filtered by database permissions. You only ever see what this user is allowed to see — never invent hidden family data.',
    'Task visibility: assignees see their assignment; the creator always sees tasks they created. Completion is per assignee. update_task / cancel_task: creator or owner/adult. acknowledge_task / complete_task: assignee only.',
